...
char* statio_get_json(time_t nowish, size_t* len)
{
    ...
    json_len += gdnsd_mon_stats_out_json(ADDVOID(buf, json_len));
    memcpy(ADDVOID(buf, json_len), json_footer, sizeof(json_footer));
    json_len += sizeof(json_footer);
    *len = json_len;
    return buf;
}
...
void statio_init(unsigned arg_num_dns_threads)
{
    ...
    json_buffer_max =
        ...
        + sizeof(json_footer);                // json_footer fixed string + NUL
